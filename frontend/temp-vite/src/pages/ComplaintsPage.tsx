...
const ComplaintsPage: React.FC<ComplaintsPageProps> = ({ customerId }) => {
  const [complaints, setComplaints] = useState<Complaint[]>([]);

  useEffect(() => {
    fetch(`http://localhost:5001/api/complaints/customer/${customerId}`, {
      headers: {
  ...
  const handleSubmit = (complaint_text: string) => {
    fetch('http://localhost:5001/api/complaints', {
      ...
      .then(res => res.json())
      .then((newComplaint: Complaint) => setComplaints([newComplaint, ...complaints]));
  };
